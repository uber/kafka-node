    ...
    var cbb = function (err, resp) {
        if (err) {
            if (self.metadataRetriesAttempts < self.options.metadataRetries) {
                ...
                if (debug) console.log('loadMetadataForTopics failed with error %s; retry attempt %d of %d', err || resp[1].error, self.metadataRetriesAttempts, self.options.metadataRetries);
...
Client.prototype.refreshBrokers = function () {
    ...
    _.values(self.brokerMetadata).forEach(function(broker) {
        var brokerAddress = broker.host + ':' + broker.port;

        if (!self.brokers.hasOwnProperty(brokerAddress)) {
            self.brokers[brokerAddress] = self.createBroker(broker.host, broker.port);
        }
        brokers[brokerAddress] = true;
    });

    Object.keys(self.brokers).forEach(function(broker) {
        if (!brokers.hasOwnProperty(broker)) {
            if (self.brokers[broker] && self.brokers[broker].close) {
...
Client.prototype.createBroker = function connect(host, port, longpolling) {
    if (debug) console.log('Broker ' + host + ':' + port + ' creating connection.');
    var self = this;
    ...
    socket.on('connect', function () {
        if (debug) console.log('Broker ' + host + ':' + port + ' has connected.');
        this.connected = true;
        self.emit('connect');
    });
